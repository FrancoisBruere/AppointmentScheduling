...
namespace AppointmentScheduling.Services
{
    ...
    {
        // add applicationDBContext to access data using dependency injection below

        private readonly ApplicationDbContext _db;

        // get emil sender using dependency injection
        ...
        public async Task<int> AddUpdate(AppointmentVM model)
        {
            ...
            if (model != null && model.Id > 0)
            {
                ...
                // update code here
                var appointment = _db.Appointments.FirstOrDefault(x => x.Id == model.Id);//retrieve appointment from db for update
            ...
            else
            {
                ...
                await _emailSender.SendEmailAsync(doctor.Email, "Appointment Created",
                    $"Your appointment with {patient.Name} @ {model.StartDate} duration {model.Duration} is created and in pending status");

                await _emailSender.SendEmailAsync(patient.Email, "Appointment Created",
                    $"Your appointment with {doctor.Name} @ {model.StartDate} duration {model.Duration} is created and in pending status");
                // email section close

                _db.Appointments.Add(appointment);
                await _db.SaveChangesAsync();
                return 2; // when adding i record 2 is identified
            }
        }
        ...
        //Implement interface to get below rightclick IappointmentIntervace
